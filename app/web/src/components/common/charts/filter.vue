  ...
  <div class="filter">
    ...
    <div class="product-wrapper" v-show="showProduct">
      ...
      <div class="pro_list" v-show="pro_filter_flag">
        <ul>
          <li v-for="(pro,index) in pro_list" :key="(pro,index)" @click="pro_toggle(pro,index)"><checkbox :isChecked="pro.selected"></checkbox><span class="name">{{pro.name}}</span></li>
        </ul>
      </div>
    </div>
  </div>
...
export default {
  ...
  data () {
    return {
      startDate: moment('2015-12-04', 'YYYY-MM-DD'),
      endDate: moment('2016-01-15', 'YYYY-MM-DD'),
